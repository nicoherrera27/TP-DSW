...
const LoginForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        const backendResponse = await fetch('http://localhost:3000/api/users/login', {
             method: 'POST',
             headers: { 'Content-Type': 'application/json' },
             body: JSON.stringify(formData)
        });
        ...
        if (token) {
            ...
            window.location.href = '/';
             ...
             console.error("LoginForm: No se recibió token del backend en la segunda llamada.");
